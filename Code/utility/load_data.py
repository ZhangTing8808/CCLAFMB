...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        with open(view_file) as f_view:
            for l in f_view.readlines():
                if len(l) == 0: break
                l = l.strip('\n')
                items = [int(i) for i in l.split(' ')]
                uid, view_items = items[0], items[1:]

                for i in view_items:
                    self.R_view[uid, i] = 1.
                self.view_set[uid]=view_items
    ...
    def get_adj_mat(self):
        try:
            ...
            adj_mat_view = sp.load_npz(self.path + '/s_adj_mat_view.npz')
            norm_adj_mat_view = sp.load_npz(self.path + '/s_norm_adj_mat_view.npz')
            mean_adj_mat_view = sp.load_npz(self.path + '/s_mean_adj_mat_view.npz')
            ...
            adj_mat_view, norm_adj_mat_view, mean_adj_mat_view = self.create_adj_mat(self.R_view)
            adj_mat_cart, norm_adj_mat_cart, mean_adj_mat_cart = self.create_adj_mat(self.R_cart)
            sp.save_npz(self.path + '/s_adj_mat.npz', adj_mat)
            sp.save_npz(self.path + '/s_norm_adj_mat.npz', norm_adj_mat)
            sp.save_npz(self.path + '/s_mean_adj_mat.npz', mean_adj_mat)

            sp.save_npz(self.path + '/s_adj_mat_view.npz', adj_mat_view)
            sp.save_npz(self.path + '/s_norm_adj_mat_view.npz', norm_adj_mat_view)
            sp.save_npz(self.path + '/s_mean_adj_mat_view.npz', mean_adj_mat_view)
        ...
        try:
            ...
            pre_adj_mat_view = sp.load_npz(self.path + '/s_pre_adj_mat_view.npz')
            ...
            rowsum = np.array(adj_mat_view.sum(1))
            d_inv = np.power(rowsum, -0.5).flatten()
            d_inv[np.isinf(d_inv)] = 0.
            d_mat_inv = sp.diags(d_inv)

            norm_adj = d_mat_inv.dot(adj_mat_view)
            norm_adj = norm_adj.dot(d_mat_inv)
            print('generate pre view adjacency matrix.')
            pre_adj_mat_view = norm_adj.tocsr()
        ...
        return pre_adj_mat,pre_adj_mat_view,pre_adj_mat_cart
