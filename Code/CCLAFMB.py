...
class CCLAFMB(object):
    ...
    def create_infoNCE_loss(self):
        # infoNCE_loss
        # Col_cl
        pos_score_bv = tf.math.exp(tf.reduce_sum(tf.multiply(self.embeddings_buy, self.embeddings_view), 1))
        pos_score_bc = tf.math.exp(tf.reduce_sum(tf.multiply(self.embeddings_buy, self.embeddings_cart), 1))

        exp_mat_bv = tf.math.exp(tf.matmul(self.embeddings_buy, tf.transpose(self.embeddings_view)))
        neg_score_bv = tf.reduce_sum(exp_mat_bv, axis=1)  # 用户和其所有负样本的和 s*1

        exp_mat_bc = tf.math.exp(tf.matmul(self.embeddings_buy, tf.transpose(self.embeddings_cart)))
        neg_score_bc = tf.reduce_sum(exp_mat_bc, axis=1)

        con_loss_Col_bv = -tf.log(1e-8 + tf.div(pos_score_bv, neg_score_bv + 1e-8))
        con_loss_Col_bc = -tf.log(1e-8 + tf.div(pos_score_bc, neg_score_bc + 1e-8))
        con_loss_Col = tf.reduce_mean(con_loss_Col_bv + con_loss_Col_bc)
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epoch):
        ...
        t1 = time()
        loss, emb_loss, infoNCE_loss = 0., 0., 0.
        ...
        for idx in range(n_batch):
            ...
            _, batch_loss, batch_emb_loss, batch_infoNCE_loss, u_embeddings_view, u_embeddings_cart, u_embeddings_buy, i_embeddings_view, i_embeddings_cart, i_embeddings_buy, u_embeddings, i_embeddings = sess.run(
                [model.opt, model.loss, model.emb_loss, model.infoNCE_loss, model.embeddings_view_all, model.embeddings_cart_all, model.embeddings_buy_all, model.embeddings_view_all_i, model.embeddings_cart_all_i, model.embeddings_buy_all_i, model.ua_embeddings, model.ia_embeddings],
